...
//   var salesTransactions = transactions.filter(transaction => transaction.type === 'sale');
...
var uniqueVendors = transactions.filter(transaction => transaction.type === 'purchase').map(transaction => transaction.vendor).filter(unique);
...
*/
var uniqueCustomers = transactions.filter(transaction => transaction.type === 'sale').map(transaction => transaction.customer).filter(unique);
...
*/
var bigSpenders = uniqueCustomers = transactions.filter(transaction => transaction.type === 'sale').filter(transaction => transaction.items.length >= 5).map(transaction => transaction.customer);
...
var sumSales = transactions.filter(transaction => transaction.type === 'sale').map(transaction => transaction.items.map(item => item.price).reduce(add, 0)).reduce(add, 0);
...
var sumPurchases = transactions.filter(transaction => transaction.type === 'purchase').map(transaction => transaction.items.map(item => item.price).reduce(add, 0)).reduce(add, 0);
...
var saleItemNumbers = function(a, b){
  return b - a
};

var mostItems = transactions.filter(transaction => transaction.type === 'sale').map(transaction => transaction.items.length).sort(saleItemNumbers)[0];
...
var purchaseItemNumbers = function(a, b){
  return a - b
};

var sumOfSmallestPurchase = transactions.filter(transaction => transaction.type === 'purchase').map(transaction => transaction.items.length).sort(purchaseItemNumbers)[0];
